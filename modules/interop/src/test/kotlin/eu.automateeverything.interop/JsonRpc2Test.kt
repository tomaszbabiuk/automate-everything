...
internal class JsonRpc2Test {
    ...
    private val target = JsonRpc2SessionHandler(repository = repositoryMock, Cbor)
    ...
    fun testFailedMethod() {
        val instancesRequest = createRequestFromType(UnknownDto::class.java, "one")
        val response = target.handleRequest(instancesRequest)
        println(response)
