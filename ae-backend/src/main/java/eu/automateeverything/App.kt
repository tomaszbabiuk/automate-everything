...
import eu.automateeverything.domain.events.NumberedEventsBus
...
open class App : ResourceConfig() {
    ...
    private val _eventsBus: EventsBus = NumberedEventsBus()
    private val repository: Repository = SqlDelightRepository()
    private val inbox: Inbox = BroadcastingInbox(_eventsBus, repository)
    private val pluginsCoordinator: PluginsCoordinator = SingletonExtensionPluginsCoordinator(_eventsBus, injectionRegistry, repository)
    private val hardwareManager = HardwareManager(pluginsCoordinator, _eventsBus, inbox, repository)
    ...
    private val automationConductor = AutomationConductor(hardwareManager, blockFactoriesCoordinator, pluginsCoordinator,
        _eventsBus, inbox, repository)
    private val pulsar = Pulsar(_eventsBus, inbox, automationConductor)
    ...
    private fun buildJsonRpc2SessionHandler(): SessionHandler<ByteArray, ByteArray> {
        ...
        val eventsSubscriptionBuilder = EventsSubscriptionBuilder(_eventsBus, eventsMapper, binaryFormat)
    ...
    private fun fillInjectionRegistry() {
        ...
        injectionRegistry.put(EventsBus::class.java, _eventsBus)
    ...
    private fun dependencyInjectionOfRest() {
        ...
        register(
            DependencyInjectionBinder(
                _eventsBus, repository, pluginsCoordinator, hardwareManager,
